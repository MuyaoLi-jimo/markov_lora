...
def lora_fine_tune(model, dataloader, num_epochs, rank, world_size,accumulation_steps, output_path):
    # 配置 LoRA 参数
...
def main(rank, world_size, model_path, output_path,training_mode='distributed',  ranks_per_gpu=16,batch_size=16,accumulation_steps=1,samedata=False,num_epochs=1,bf16=False):
    ...
    if training_mode=="bias_tuning":
        if not samedata:
            sampler = DistributedSampler(tokenized_datasets, num_replicas=world_size, rank=rank, shuffle=True)
            dataloader = DataLoader(
                tokenized_datasets,
                drop_last=True,
        ...
        #model = PeftModel.from_pretrained(model, model_path, subfolder = f"{model_path}/pissa_init", is_trainable=True)
        ...
        lora_fine_tune(model, dataloader, num_epochs, rank, world_size,accumulation_steps, output_path)
